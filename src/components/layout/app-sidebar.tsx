...
export function AppSidebar({ collapsible = false, onNavigate }: { collapsible?: boolean; onNavigate?: () => void }) {
  ...
  return <aside aria-label="Navegação principal" className={cn("flex h-full flex-col border-r border-[var(--border)] bg-white px-3 py-4 transition-[width] duration-200 ease-out", collapsed && collapsible ? "w-[72px]" : "w-[260px]")}>
    <div className={cn("mb-6 flex h-9 items-center", collapsed && collapsible ? "justify-center" : "justify-between")}>
      <Link href="/dashboard" aria-label="ClassHub, visão geral" className={cn("flex h-9 min-w-0 items-center rounded-lg text-[15px] font-semibold tracking-[-0.02em] text-[var(--foreground)] transition-colors duration-150 ease-out hover:bg-[var(--surface-blue-soft)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--primary)]", collapsed && collapsible ? "justify-center px-1" : "gap-2.5 px-2")}><span aria-hidden="true" className="grid size-7 shrink-0 place-items-center rounded-lg bg-[var(--primary)] text-[11px] font-semibold text-white transition-colors duration-150 ease-out hover:bg-[var(--primary-hover)]">C</span><span className={cn("overflow-hidden whitespace-nowrap transition-[width,opacity] duration-150 ease-out", collapsed && collapsible ? "w-0 opacity-0" : "w-[92px] opacity-100")}>ClassHub</span></Link>
