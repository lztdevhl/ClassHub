...
export async function getReportData(params: ReportParams) {
  ...
  const students = new Set(lessons.map((lesson) => lesson.student.id)).size;
  return { lessons, summary: { students, total: lessons.length, present, absent: lessons.length - present, attendancePercent: percent(present, lessons.length), completed, pending: lessons.filter((lesson) => lesson.activityCompleted === false).length } };
}
